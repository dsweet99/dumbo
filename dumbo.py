...
def acquisition_function(y_ex, y_var):
    ...
    Seek points that have high value (y_ex) but also
     high uncertainty (y_var).
    Put another way: Balance exploration (y_var) with
     exploitation (y_ex).
...
def optimize(x_m, y_m, eps=0.1, num_iterations=1000):
    """Random search
    ...
    for _ in range(num_iterations):
        x = np.minimum(1.0, np.maximum(0.0, x_best + eps * np.random.uniform(size=(num_dim,))))
        x = np.atleast_2d(x)
